...
const AddSuspect = ({getOffenceTicketId}:any) => {
  // ** States
  const [show, setShow] = useState<boolean>(false)
  const [basicPicker, setBasicPicker] = useState<Date | null>(new Date())
  const {  setsuspectCount } = React.useContext<any>(OrganisationContext)
  const { addsuspectData, setaddsuspectData} = React.useContext<any>(TicketContext)
  ...
  /* useEffect(() => {
    const localData = JSON.parse(localStorage.getItem('suspectData') || '{}')
    setAllLocalData(localData)
    setLoader(false)
    setEditFlagSuspect(false)

  }, [loader,editflagsuspect]) */
