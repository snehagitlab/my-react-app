...
const OtherOffenceInfo = () => {
  ...
  const { othercount, setotherCount } = React.useContext<any>(OrganisationContext)
  ...
  const handleCreateTicket = async () => {
    ...
    setAllData([...allData, requestData])

    // console.log(allData && allData + '<============')

    localStorage.setItem(
      'otherOffenceData',
      JSON.stringify(allData && allData.length > 0 ? [...allData, requestData] : [requestData])
    )
  ...
  useEffect(() => {
    const localData = JSON.parse(localStorage.getItem('otherOffenceData') || '{}')
        ...
        <Grid item>
          <TableContainer sx={{ maxHeight: 440 }}>
            <Table stickyHeader aria-label='sticky table'>
              ...
              <TableBody>
                {allLocalData.length > 0 ? (
                  allLocalData.map((item: any, id: any) => {
                    return (
                      <>
                        <TableRow key={id} hover role='checkbox' tabIndex={-1}>
                          <TableCell key={id}>{item.name}</TableCell>
                ...
                <Grid item sm={6} xs={12}>
                  <FormControl fullWidth>
                    ...
                    >
                      ...
                      <MenuItem value='Native Americans and Alaska Natives'>Native Americans and Alaska Natives</MenuItem>
...
export default OtherOffenceInfo